...
<form method="POST" action="<?= $currentUrl; ?>" enctype="multipart/form-data">
		...
		<td>
			...
			<select name="option_MODULE_MODE">
				...
				<? foreach ($arModuleMode as $mode) : ?>
					...
					<option value="<?= $mode ?>" <?=($option['MODULE_MODE'] == $mode ) ? 'selected': '' ?>><?= Loc::getMessage('ISPRO_ING2PICTURE_MODULE_MODE_' . $mode) ?></option>
				<? endforeach ?>
			</select>
		</td>
	</tr>
	...
	<tr>
		<td colspan="2">
			<input type="submit" class="adm-btn-save" name="saveoptions" value="<? echo Loc::getMessage('ISPRO_ING2PICTURE_SAVE'); ?>">
		</td>
	</tr>
