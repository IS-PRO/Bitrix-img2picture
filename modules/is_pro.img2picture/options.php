...
<form method="POST" action="<?= $currentUrl; ?>" enctype="multipart/form-data" id="img2picture_form">
	...
	<? foreach ($siteIds as $sId => $sName) : ?>
					...
					<? elseif ($option_name_def == 'RESPONSIVE') : ?>
						...
						<table width="100%">
							...
							<? foreach ($option[$option_name . '_VALUE'] as $key => $val) : ?>
								<tr>
									<td>
										<input type="number" name="option_<?= $option_name ?>[<?= $key ?>][min]" value="<?= $val['min'] ?>">
									</td>
									<td>
										<input type="number" name="option_<?= $option_name ?>[<?= $key ?>][max]" value="<?= $val['max'] ?>">
									</td>
									<td>
										<input type="number" name="option_<?= $option_name ?>[<?= $key ?>][width]" value="<?= $val['width'] ?>">
									</td>
								</tr>
							<? endforeach ?>
						</table>
		...
		<tr>
			...
			<td>
				<button type="submit" class="adm-btn" name="save" value="reset_<?= $sId ?>"><?= Loc::getMessage('ISPRO_IMG2PICTURE_DEFAULT'); ?> (<?= $sName ?>)</button>
			</td>
		</tr>
	<? endforeach ?>
	...
	<button type="submit" class="adm-btn adm-btn-save" name="save" value="save"><? echo Loc::getMessage('ISPRO_IMG2PICTURE_SAVE'); ?></button>
	<button type="submit" class="adm-btn adm-btn-save" name="save" value="removefiles"><? echo Loc::getMessage('ISPRO_IMG2PICTURE_REMOVE_FILES'); ?></button>
